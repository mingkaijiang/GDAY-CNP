...
def main(experiment_id, site, SPIN_UP=True, POST_INDUST=True, AMB_AVG=True, ELE_AVG=True):
    ...
    if SPIN_UP == True:
        ...
        replace_dict = {
                        ...
                        "jmaxna": "50.824",      # forcing intercept to zero; TPU not removed: 49.93
                        ...
                        "vcmaxna": "27.684",     # forcing intercept to zero; TPU not removed: 27.707
                        "vcmaxpa": "517.11",     # forcing intercept to zero; TPU not removed: 516.83
                        ...
                        "pmincrit": "2.0",
                        "prateloss": "0.1",
                        "prateuptake": "3.0",    # Reduce to get a P limitation effect
                        "slowpcmin": "0.005",
                        "slowpcmax": "0.011111",
    ...
    if POST_INDUST == True:
        ...
        out_fn = "%s_amb_01_ellsworth_NP_tp.csv" % (site)
        out_param_fname = os.path.join(param_dir, otag + ".cfg")
        cfg_fname = os.path.join(param_dir, itag + ".cfg")
        met_fname = os.path.join(met_dir, mtag)
        out_fname = os.path.join(run_dir, out_fn)
    ...
    if AMB_AVG == True:
        ...
        out_fn = "%s_amb_avg_02_ellsworth_NP_tp.csv" % (site)
        out_param_fname = os.path.join(param_dir, otag + ".cfg")
        cfg_fname = os.path.join(param_dir, itag + ".cfg")
        met_fname = os.path.join(met_dir, mtag)
        out_fname = os.path.join(run_dir, out_fn)
    ...
    if ELE_AVG == True:
        ...
        out_fn = "%s_ele_avg_03_ellsworth_NP_tp.csv" % (site)
        out_param_fname = os.path.join(param_dir, otag + ".cfg")
        cfg_fname = os.path.join(param_dir, itag + ".cfg")
        met_fname = os.path.join(met_dir, mtag)
        out_fname = os.path.join(run_dir, out_fn)
