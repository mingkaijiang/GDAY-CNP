...
def load_gday_output(fname, nfix):
    ...
    return {'YEAR':year, 'DOY':doy, 'SW':pawater_root,
            ...
            'PCON':pcon, 'PCAN':shootp, 'PWOOD':cwp,
            'PCR':crootp, 'PFR':rootp,
            'PSTOR':pstorage, 
            'PLIT':litterpag, 'PRLIT':litterpbg, 
            'PSOIL':psoil, 'PPOOLM':inorgp, 'PPOOLLAB':inorglabp,
            'PPOOLSORB':inorgsorbp, 'PPOOLAVL':inorgavlp, 'PPOOLSSORB':inorgssorbp, 
            'PPOOLOCC':inorgoccp, 'PPOOLPAR':inorgparp,
            'PPOOLO':ppoolo, 
            'PLITIN':deadleafp, 'PWLIN':pdeadwood, 'PCRLIN':deadcrootp,
            'PFRLIN':deadrootp, 'PUP':pup,
            'PGMIN':pgross, 'PMIN':pmin, 'PLEACH':pleach,
            'PGL':ppleaf, 'PGW':pwood_growth, 'PGCR':ppcroot, 'PGR':pproot}, git_ver
...
def setup_varnames():
    variable = ['Year', 'Day of the year', 'CO2', 'Precipitation', 'PAR',
                    ...
                    'Fine Root retranslocation', 'P Conc Leaves', 'P Mass Leaves',
                    'P Mass Wood','P Mass Coarse Roots', 'P Mass Fine Roots',
                    'P storage', 
                    'P litter aboveground', 'P litter belowground',
                    'P Soil Total', 'P Inorganic pool', 'P labile pool',
                    'P sorbed pool', 'P inorganic available pool', 'P strongly sorbed pool',
                    'P occluded pool', 'P parent pool',
                    'P in Organic form','P Leaf Litterfall', 'P Wood/brch litterfall', 
                    'P Coarse Root litter input','P Fine Root litter input', 'P Biomass Uptake',
    ...
    variable_names = ['YEAR', 'DOY', 'CO2', 'PPT', 'PAR', 'AT', 'ST', 'VPD',
                        ...
                        'PCR', 'PFR','PSTOR', 
                        'PLIT', 'PRLIT',
                        'PSOIL', 'PPOOLM', 'PPOOLLAB',
                        'PPOOLSORB', 'PPOOLAVL', 'PPOOLSSORB',
                        'PPOOLOCC', 'PPOOLPAR',
                        'PPOOLO', 
                        'PLITIN', 'PWLIN', 'PCRLIN',
                        'PFRLIN', 'PUP',
